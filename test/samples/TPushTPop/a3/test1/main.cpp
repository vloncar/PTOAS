...
void LaunchMatmulTPushPopPrint(uint8_t *a, uint8_t *b, uint8_t *gmSlotBuffer,
                               void *stream);
...
int main() {
  ...
  LaunchMatmulTPushPopPrint(devA, devB, devSlotBuffer, stream);
  ACL_CHECK(aclrtSynchronizeStream(stream));

  std::puts(
      "Kernel finished. Expect TPRINT output for the 16x16 Vec tile to be "
