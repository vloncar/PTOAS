...
"""TDequant int16 kernel sample.

  tdequant(src_i16, scale_f32, offset_f32) -> dst_f32

Loads a 32x32 i16 tile (src), a 32x8 f32 per-row scale tile, and a 32x8 f32
...
The scale and offset tiles are row vectors: only the first column of each row
is used, broadcast across all output columns.

Note: i16 tiles require Cols*sizeof(T) to be a multiple of 32 bytes
(the NPU aligned-size). At 2 bytes/element that means Cols >= 16.
...
# Tile shapes
_SRC_SHAPE   = [32, 32]   # i16 source tile
_PARA_SHAPE  = [32, 8]    # f32 parameter tile (scale / offset), 8 cols minimum
...
def _make_common_types(ctx):
    ...
    ptv_f32_src  = pto.PartitionTensorViewType.get(_SRC_SHAPE,  f32, ctx)
    ptv_i16_src  = pto.PartitionTensorViewType.get(_SRC_SHAPE,  i16, ctx)
    ...
    tb_i16  = pto.TileBufType.get(_SRC_SHAPE,  i16, vec, _SRC_SHAPE,  cfg, ctx)
...
def build():
    with Context() as ctx:
        ...
        with Location.unknown(ctx):
            ...
            # ------------------------------------------------------------------
            # @tdequant_kernel(src_ptr:    !pto.ptr<i16>,
            #                  scale_ptr:  !pto.ptr<f32>,
            #                  offset_ptr: !pto.ptr<f32>,
            #                  dst_ptr:    !pto.ptr<f32>)
            # ------------------------------------------------------------------
            fn_ty = func.FunctionType.get(
                [t.ptr_i16, t.ptr_f32, t.ptr_f32, t.ptr_f32], []
            )
            with InsertionPoint(m.body):
                fn = func.FuncOp("tdequant_kernel", fn_ty)
            ...
            with InsertionPoint(entry):
                ...
                tv_src    = pto.MakeTensorViewOp(
                    t.tv2_i16,  src_ptr,    [c32, c32], [c32, c1]
                ).result
                ...
                sv_src    = pto.PartitionViewOp(
                    t.ptv_i16_src,  tv_src,    offsets=[c0, c0], sizes=[c32, c32]
                ).result
                ...
                tb_src    = pto.AllocTileOp(t.tb_i16).result
